"""
Script to enroll student with ID "190001" in courses and fix Degree Planning page issues.

This script:
1. Checks if student "190001" exists
2. Creates enrollment records for the student
3. Ensures proper enrollment status and timestamps
4. Fixes Degree Planning page issues by ensuring proper data structure
"""
...
def enroll_student_in_courses(student_id, course_ids=None):
    ...
    if course_ids is None:
        # Get some courses from the same department or general courses
        department_courses = Course.objects.filter(department=student.department)[:3]
        general_courses = Course.objects.filter(department__icontains='general')[:2]
        ...
        if not course_ids:
            # If no courses found, get any 5 courses
